...
function uniqueOflPath(
  ...
  while (taken.has(candidate)) {
    candidate = normalizePath(
      joinPath(
        joinPath(OFL_FIXTURES_DIR, manufacturerKey),
        `${fixtureKey}_${index}.json`,
      ),
    );
    index += 1;
  }
...
export function collectOflPaths(
  fixtures: Array<{ ofl?: { filePath: string } }>,
): string[] {
